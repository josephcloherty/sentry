...
SITL_TELEMETRY_STREAM_ENABLED = False
...
# TEST mode (fake telemetry from test_mode_telemetry.py)
# Connect to cameras on ports 8886/8887/8885 and
# receive telemetry over WebSocket on port 8888.
SITL_MODE = False
TEST_MODE = True
...
async def receive_telemetry():
    """Receive telemetry. In TEST mode: WebSocket on TELEMETRY_PORT (from
    test_mode_telemetry.py). In SITL mode: SSE stream or /status polling."""
    ...
    while True:
        try:
            if SITL_MODE and SITL_TELEMETRY_STREAM_ENABLED:
            ...
            # TEST mode: WebSocket telemetry from test_mode_telemetry.py
            try:
                async with websockets.connect(f'ws://{SERVER_IP}:{TELEMETRY_PORT}') as ws:
                    print(f"Connected to TEST telemetry ws://{SERVER_IP}:{TELEMETRY_PORT}")
                    async for raw in ws:
                        try:
                ...
                print(f"TEST telemetry connect failed: {e}")
            except Exception as e:
                print(f"TEST telemetry error: {e}")
...
def index():
    ...
    effective_test_mode = bool(TEST_MODE) or bool(user_settings.get('test_mode'))
...
def _post_sitl_command(payload, *, require_link=False, fail_message='Failed to send command'):
    if not SITL_MODE:
        # TEST mode: no SITL HTTP endpoint. Forward via command_ws if available.
        try:
            ws = command_ws
            if ws is not None and TEST_MODE_ENABLE_COMMAND_WS:
                asyncio.run(ws.send(json.dumps(payload)))
                return True, None, None, None
        except Exception as e:
            if require_link:
                return False, fail_message, 500, None
            print(f'[Flask] TEST-mode command forward failed: {e}')
        return True, None, None, None
    try:
...
if __name__ == '__main__':
    ...
    if SITL_MODE:
        ...
        print(f"TEST_MODE: expecting fake telemetry on ws://{SERVER_IP}:{TELEMETRY_PORT} "
              f"and cameras on {CAM0_PORT}/{CAM1_PORT}/{CAM_HQ_PORT}")
