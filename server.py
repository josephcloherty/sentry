        ...
        if fid and fid.locked:
            ...
            if hasattr(fid, 'corners') and getattr(fid, 'corners') is not None:
                try:
                    ...
                    pass
            id_text = f"Tag ID: {fid.fiducial_id if fid.fiducial_id is not None else '?'}"
            conf_text = f"Conf: {fid.confidence:.2f}"
            cv2.putText(display_frame, id_text, (10, VIDEO_HEIGHT-20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
            cv2.putText(display_frame, conf_text, (10, VIDEO_HEIGHT-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        else:
            # Fallback to IR
